...
# todo: У меня почему-то первый игрок в сохранении меняется ровно тогда когда взят тайл первого игрока.
#  Первый игрок должен меняться после выставление плиток на поле.
#  При этом при открытии клиента активный игрок показывается корректно.
#  Потому что там Первый игрок берется из информации Активного игрока
# todo: Сохранение данных в csv о сделанном ходе игрока. Данные которые передались от клиента серверу